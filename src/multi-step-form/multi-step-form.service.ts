...
export class MultiStepFormService {
  ...
  async findOne(email: string):Promise<Application>{
    try {
      const res = await this.prismaService.application.findUnique({where:{email:email}});
      if(res){
        return res;
      }
      throw new NotFoundException(`Could not find application details with this ${email} email`);
    } catch (error) {
  ...
  async applyForm(formValue:Application,step:number):Promise<{msg:string,application:Application}>{
    console.log(step);
    if(step <= 3 && step >=1){
      const value = await validateSchema(formValue,step)
      // switch case
      switch (step) {
        case 1: return this.savePersonal(value);
          break;
        case 2: return await this.saveEducation(value);
          break;
        case 3: return await this.saveInternship(value);
          break;
        default: return;
          break;
      }
  ...
  async savePersonal(value:Application){
    try {
      const res = await this.prismaService.application.create({data:{...value,prevStep:0,currStep:1,nextStep:2}})
      ...
      if(e instanceof Prisma.PrismaClientKnownRequestError){
        if(e.code === "P2002"){
          console.log(e)
          throw new HttpException(`${e.meta?.target[0]} Already Exists`,HttpStatus.CONFLICT)
        }
  ...
  async saveInternship(value:Application){
      try {
        const res = await this.prismaService.application.update({
          data:{...value,prevStep:2,currStep:3,nextStep:4,status:"DONE"},
          where:{email:value.email}})
        return {
